...
#if defined(CONFIG_EXYNOS_THERMAL_V2) || defined(CONFIG_EXYNOS_THERMAL_V2_MODULE)
		...
		unsigned long val, void *v)
{
	...
	if (v == NULL) {
		GPU_LOG(DVFS_ERROR, DUMMY, 0u, 0u, "%s: pm_qos_class pointer is NULL", __func__);
		return NOTIFY_BAD;
	}

	pm_qos_class = *((int *)v);

	GPU_LOG(DVFS_INFO, DUMMY, 0u, 0u, "%s: update GPU PM Qos class %d to %ld kHz\n", __func__, pm_qos_class, val);
