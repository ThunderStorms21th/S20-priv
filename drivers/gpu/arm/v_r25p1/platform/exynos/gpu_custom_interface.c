...
static ssize_t show_volt_table(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	max = gpu_dvfs_get_stock_level(platform->gpu_max_clock);
	min = gpu_dvfs_get_stock_level(platform->gpu_min_clock);
...
{
	struct exynos_context *platform = (struct exynos_context *)pkbdev->platform_context;
	int max = gpu_dvfs_get_stock_level(platform->gpu_max_clock);
	int min = gpu_dvfs_get_stock_level(platform->gpu_min_clock);
...
static ssize_t set_gpu_throttling5(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if ((throttling5 < platform->gpu_min_clock) || (throttling5 > platform->gpu_max_clock)) {
...
#define SUSTAINABLE_FREQ 455000 // KHz
...
static ssize_t show_axuser_hint(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t ret = 0;
	phys_addr_t sysreg_g3d_base;
	void __iomem *reg;
	u64 val;
	struct exynos_context *platform = (struct exynos_context *)pkbdev->platform_context;
	...
	if (!platform)
		return -ENODEV;
	sysreg_g3d_base = platform->sysreg_g3d;

	reg = ioremap(sysreg_g3d_base + 0x0420, SZ_8K); /* SYSREG_G3D + GPU_ARUSER(0x0420)*/
	val = __raw_readl(reg);
	ret += scnprintf(buf + ret, PAGE_SIZE - ret, "aruser[0x%llx] = 0x%llx\n", sysreg_g3d_base + 0x0420, val);
	iounmap(reg);

	reg = ioremap(sysreg_g3d_base + 0x0440, SZ_8K); /* SYSREG_G3D + GPU_AWUSER(0x0440)*/
	val = __raw_readl(reg);
	ret += scnprintf(buf + ret, PAGE_SIZE - ret, "awuser[0x%llx] = 0x%llx\n", sysreg_g3d_base + 0x0440, val);
	iounmap(reg);

	return ret;
}

static ssize_t set_awuser_hint(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	ssize_t ret = 0;
	phys_addr_t sysreg_g3d_base;
	phys_addr_t offset = 0x0440;
	void __iomem *reg;
	u64 val;
	struct exynos_context *platform = (struct exynos_context *)pkbdev->platform_context;
	...
	sysreg_g3d_base = platform->sysreg_g3d;
	reg = ioremap(sysreg_g3d_base + offset, SZ_8K);
...
static ssize_t set_aruser_hint(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	ssize_t ret = 0;
	phys_addr_t sysreg_g3d_base;
	phys_addr_t offset = 0x0420;
	void __iomem *reg;
	u64 val;
	struct exynos_context *platform = (struct exynos_context *)pkbdev->platform_context;
...
DEVICE_ATTR(aruser_hint, S_IRUGO|S_IWUSR, show_axuser_hint, set_aruser_hint);
DEVICE_ATTR(awuser_hint, S_IRUGO|S_IWUSR, show_axuser_hint, set_awuser_hint);
