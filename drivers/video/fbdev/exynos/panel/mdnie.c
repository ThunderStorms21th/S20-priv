...
static int mdnie_get_efs(char *filename, int *value)
{
	...
	if (rc != 3) {
		panel_err("failed to kstrtoint %d\n", rc);
		ret = -EINVAL;
		goto exit;
	}

	panel_info("%s(size %d) : %d %d %d\n",
			filename, fsize, value[0], value[1], value[2]);
...
int mdnie_probe(struct panel_device *panel, struct mdnie_tune *mdnie_tune)
{
	...
	if (unlikely(!mdnie_tune)) {
		panel_err("mdnie_tune not exist\n");
		return -ENODEV;
	}
	...
	const char *name, const char *buf, size_t size)
{
	...
	if (!class)
		return -EINVAL;
	if (!class->p) {
		WARN(1, "called for class '%s' before it was initialized",
				class->name);
		return -EINVAL;
	}
