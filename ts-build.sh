...
DEFCONFIG2=exynos9830-x1sxx_defconfig
DEFCONFIG3=exynos9830-y2sxxx_defconfig
...
BUILD_KERNEL_980F()
{
    ...
    if [ -e arch/arm64/boot/Image ]; then
      ...
      $(pwd)/tools/mkdtimg cfg_create $(pwd)/out/SM-980F/dtbo.img dt.configs/x1sxx.cfg -d ${DTB_DIR}/samsung
    else
      echo "return to Main menu' 'Kernel STUCK in BUILD!"
    fi
}
...
BUILD_KERNEL_985F()
{
    ...
    if [ -e arch/arm64/boot/Image ]; then
      ...
      $(pwd)/tools/mkdtimg cfg_create $(pwd)/out/SM-985F/dtbo.img dt.configs/y2sxxx.cfg -d ${DTB_DIR}/samsung
    else
      echo "return to Main menu' 'Kernel STUCK in BUILD!"
    fi
}
