...
void synchronize_irq(unsigned int irq)
{
	...
	if (desc) {
		secdbg_base_set_task_in_sync_irq((uint64_t)current, irq, (desc && desc->action) ? desc->action->name : NULL, desc);
		...
		secdbg_base_set_task_in_sync_irq(0, 0, 0, 0);
	}
}
...
static void affine_one_perf_thread(struct task_struct *t)
{
	...
	set_cpus_allowed_ptr(t, cpu_prime_mask);
}
...
void reaffine_perf_irqs(void)
{
	...
	list_for_each_entry(data, &perf_crit_irqs.list, list) {
		...
		raw_spin_lock_irqsave(&desc->lock, flags);
		irq_set_affinity_locked(&desc->irq_data, cpu_prime_mask, true);
		if (desc->action->thread)
